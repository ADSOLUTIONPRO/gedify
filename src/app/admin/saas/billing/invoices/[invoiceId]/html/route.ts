...
/** Sert le HTML de la facture. Superuser OU owner du tenant de la facture. */
export async function GET(_req: NextRequest, { params }: Ctx) {
  ...
  const buf = await readInvoiceFile(data.invoice, "html");
  if (!buf) return NextResponse.json({ error: "Document indisponible (facture non émise ?)." }, { status: 404 });
  return new NextResponse(new Uint8Array(buf), { headers: { "Content-Type": "text/html; charset=utf-8" } });
}
