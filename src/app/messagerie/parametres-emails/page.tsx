...
/**
 * « Paramètres des Emails » — implémentation UNIQUE, intégrée à l'espace Mails
 * (layout messagerie/layout.tsx → sidebar Mails). L'ancienne route /emails
 * redirige ici. Regroupe comptes connectés (IMAP + Google OAuth) et signatures.
 * Les réglages techniques globaux (OAuth app, secrets) restent en Administration.
 */
...
export default async function MessagerieParametresEmailsPage() {
  ...
  return (
    <div className="mx-auto w-full max-w-5xl space-y-4 p-4 lg:p-6">
      <PageHeader
        compact
        breadcrumb={[{ href: "/dashboard", label: "Accueil" }, { href: "/messagerie/inbox", label: "Mails" }, { label: "Paramètres des Emails" }]}
        title="Paramètres des Emails"
        description="Gérez vos comptes, signatures, synchronisations et préférences de messagerie."
        actions={
