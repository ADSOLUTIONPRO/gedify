import { createRequire as __gedifyCreateRequire } from 'module'; const require = __gedifyCreateRequire(import.meta.url);

// scripts/duplicates-doctor.ts
import { existsSync, readFileSync } from "node:fs";
import path2 from "node:path";

// scripts/_shared.ts
import path from "node:path";
function dataDir() {
  return process.env.JSON_DATA_DIR?.trim() || process.env.DATA_DIR?.trim() || process.env.APP_DATA_DIR?.trim() || path.join(process.cwd(), ".data");
}

// scripts/duplicates-doctor.ts
function normTitle(d) {
  return (d.original_file_name ?? d.title ?? "").toLowerCase().normalize("NFD").replace(/[̀-ͯ]/g, "").replace(/\.[a-z0-9]+$/, "").replace(/[^a-z0-9]+/g, " ").trim();
}
function main() {
  const root = dataDir();
  const file = path2.join(root, "engine", "documents.json");
  let docs = [];
  if (existsSync(file)) {
    try {
      const parsed = JSON.parse(readFileSync(file, "utf8"));
      docs = Array.isArray(parsed) ? parsed : [];
    } catch {
      docs = [];
    }
  }
  docs = docs.filter((d) => !d.deleted);
  const byHash = /* @__PURE__ */ new Map();
  for (const d of docs) {
    if (!d.checksum) continue;
    const a = byHash.get(d.checksum) ?? [];
    a.push(d);
    byHash.set(d.checksum, a);
  }
  const exact = [...byHash.values()].filter((a) => a.length > 1);
  const exactIds = new Set(exact.flat().map((d) => d.id));
  const byKey = /* @__PURE__ */ new Map();
  for (const d of docs) {
    if (exactIds.has(d.id)) continue;
    const t = normTitle(d);
    if (t.length < 3) continue;
    const key = `${t}|${d.page_count ?? "?"}`;
    const a = byKey.get(key) ?? [];
    a.push(d);
    byKey.set(key, a);
  }
  const probable = [...byKey.values()].filter((a) => a.length > 1);
  console.log(`
\u{1F4C2} Data-dir : ${root}`);
  console.log(`\u{1F4C4} Documents actifs : ${docs.length}
`);
  console.log(`\u2500\u2500 Doublons exacts (m\xEAme empreinte) : ${exact.length} groupe(s) \u2500\u2500`);
  for (const g of exact.slice(0, 30)) {
    console.log(`  ${g.map((d) => `#${d.id}`).join(", ")} \u2014 ${g[0].title ?? ""}`);
  }
  console.log(`
\u2500\u2500 Doublons probables (nom + pages proches) : ${probable.length} groupe(s) \u2500\u2500`);
  for (const g of probable.slice(0, 30)) {
    console.log(`  ${g.map((d) => `#${d.id}`).join(", ")} \u2014 ${g[0].title ?? ""}`);
  }
  console.log("");
}
main();
