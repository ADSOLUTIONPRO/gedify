...
// scripts/saas/attach-existing-data-to-tenant.ts
var TENANT_ID = process.env.TENANT_ID?.trim() || "azserver-staging";
var TABLES = [
...
async function main() {
  ...
  try {
    try {
      ...
      if (t.rowCount === 0) {
        console.warn(`\u26A0\uFE0F  Tenant \xAB ${TENANT_ID} \xBB introuvable \u2014 lancez d'abord: npm run saas:init-tenant`);
      }
    } catch {
      console.warn("\u26A0\uFE0F  Table tenants absente \u2014 lancez: npm run db:push && npm run saas:init-tenant");
    }
    console.log(`Rattachement des donn\xE9es existantes au tenant \xAB ${TENANT_ID} \xBB :`);
    const summary = [];
    for (const { table, label } of TABLES) {
      try {
        ...
        console.log(`   \u2022 ${label.padEnd(34)}: ${attached} rattach\xE9(s)`);
      } catch (e) {
        console.warn(`   \u2022 ${label.padEnd(34)}: ignor\xE9e (${e instanceof Error ? e.message : String(e)})`);
      }
    }
    const total = summary.reduce((s, x) => s + x.attached, 0);
    console.log(`\u2705 saas:attach-data termin\xE9 (idempotent) \u2014 ${total} ligne(s) rattach\xE9e(s) au total.`);
  } finally {
    await client.end();
  }
}
