...
// scripts/saas/check-subscriptions.ts
async function main() {
  ...
  try {
    ...
    for (const t of tenants.rows) {
      ...
      const sres = await client.query("SELECT status FROM subscriptions WHERE tenant_id=$1 ORDER BY created_at DESC LIMIT 1", [tenantId]).catch(() => ({ rows: [] }));
    ...
    console.log("");
    console.log(`R\xE9sum\xE9 : actifs=${active} trialing=${trialing} past_due=${pastDue} canceled=${canceled} sans_abo=${none}`);
    if (issues.length === 0) {
      console.log("\u2705 Abonnements coh\xE9rents.");
      process.exit(0);
    } else {
...
main().catch((e) => {
  console.error("\u274C saas:check-subscriptions :", e instanceof Error ? e.message : String(e));
  process.exit(1);
});
