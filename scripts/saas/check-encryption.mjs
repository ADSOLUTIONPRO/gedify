...
function unwrap(kek, wrapped, tenantId) {
  try {
...
async function main() {
  ...
  try {
    ...
    if (kek && keyRows.length > 0) {
      ...
      console.log(`   \u2022 DEK d\xE9wrappables (KEK) : ${ok}/${keyRows.length}`);
      if (bad > 0) console.log(`   \u274C ${bad} cl\xE9(s) NON d\xE9chiffrable(s) avec la KEK courante \u2014 KEK erron\xE9e ou chang\xE9e ! Les fichiers de ces tenants seront illisibles.`);
      else console.log("   \u2705 Toutes les cl\xE9s tenant correspondent \xE0 la KEK courante.");
    } else if (!kek && keyRows.length > 0) {
      console.log("   \u26A0\uFE0F  Des cl\xE9s tenant existent mais aucune KEK valide n'est configur\xE9e : fichiers chiffr\xE9s illisibles tant que la KEK n'est pas fournie.");
    }
