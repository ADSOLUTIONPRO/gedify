...
// scripts/saas/check-entitlements.ts
function activeGrant(rows) {
  const now = Date.now();
  for (const g of rows) {
...
async function main() {
  ...
  const issues = [];
  const known = /* @__PURE__ */ new Set(["free", "test", "pro", "business", "internal"]);
  try {
    try {
    ...
    console.log("");
    if (issues.length === 0) {
      console.log("\u2705 Droits effectifs coh\xE9rents.");
      process.exit(0);
    }
    console.error("\u274C Incoh\xE9rences :");
...
main().catch((e) => {
  console.error("\u274C saas:check-entitlements :", e instanceof Error ? e.message : String(e));
  process.exit(1);
});
