...
// scripts/saas/seed-mail-templates.ts
...
// src/lib/saas/mailing/templates.ts
var p = (s) => `<p style="margin:0 0 14px">${s}</p>`;
var btn = (href, label) => `<p style="margin:22px 0"><a href="${href}" style="background:#0E7490;color:#fff;text-decoration:none;padding:11px 20px;border-radius:8px;display:inline-block;font-weight:700">${label}</a></p>`;
var greet = "{{recipientName}}";
var DEFAULT_TEMPLATES = [
  // ── Compte / accès ──────────────────────────────────────────────────────
  {
    ...
    name: "Bienvenue",
    category: "account",
    subject: "Bienvenue sur {{appName}} \u{1F389}",
    html: p(`Bonjour ${greet},`) + p("Votre espace <strong>{{tenantName}}</strong> est pr\xEAt.") + btn("{{appUrl}}", "Acc\xE9der \xE0 mon espace"),
    variables: ["recipientName", "appName", "tenantName", "appUrl"]
  },
  {
    key: "account.invitation",
    name: "Invitation \xE0 rejoindre un espace",
    category: "account",
    subject: "Vous \xEAtes invit\xE9(e) \xE0 rejoindre {{tenantName}}",
    html: p(`Bonjour,`) + p("{{inviterName}} vous invite \xE0 rejoindre l'espace <strong>{{tenantName}}</strong>.") + btn("{{inviteUrl}}", "Accepter l'invitation"),
  ...
  },
  {
    ...
    html: p(`Bonjour ${greet},`) + p("Cliquez ci-dessous pour d\xE9finir un nouveau mot de passe. Ce lien expire dans {{expiresIn}}.") + btn("{{resetUrl}}", "R\xE9initialiser"),
    variables: ["recipientName", "resetUrl", "expiresIn"]
  },
  {
    key: "account.email_verification",
    name: "V\xE9rification d'adresse email",
    category: "account",
  ...
  },
  {
    ...
    category: "account",
    subject: "Nouvelle connexion \xE0 votre compte",
    html: p(`Bonjour ${greet},`) + p("Une connexion a eu lieu le {{date}} depuis {{location}}. Si ce n'\xE9tait pas vous, changez votre mot de passe imm\xE9diatement."),
    variables: ["recipientName", "date", "location"]
  },
  // ── Abonnement ──────────────────────────────────────────────────────────
  {
    key: "subscription.activated",
    name: "Abonnement activ\xE9",
    category: "subscription",
    subject: "Votre abonnement {{planName}} est actif",
    html: p(`Bonjour ${greet},`) + p("Votre abonnement <strong>{{planName}}</strong> est d\xE9sormais actif. Prochaine \xE9ch\xE9ance : {{nextBillingDate}}."),
    variables: ["recipientName", "planName", "nextBillingDate"]
  },
  {
    key: "subscription.trial_started",
    name: "D\xE9but de p\xE9riode d'essai",
    category: "subscription",
    subject: "Votre essai gratuit a commenc\xE9",
    html: p(`Bonjour ${greet},`) + p("Votre p\xE9riode d'essai de {{planName}} est ouverte jusqu'au {{trialEnd}}. Profitez-en !"),
  ...
  },
  {
    ...
    category: "subscription",
    subject: "Votre essai se termine bient\xF4t",
    html: p(`Bonjour ${greet},`) + p("Votre essai se termine le {{trialEnd}}. Choisissez une offre pour ne pas perdre l'acc\xE8s.") + btn("{{pricingUrl}}", "Choisir une offre"),
  ...
  },
  {
    ...
    category: "subscription",
    subject: "Votre offre a \xE9t\xE9 mise \xE0 jour",
    html: p(`Bonjour ${greet},`) + p("Votre offre est pass\xE9e \xE0 <strong>{{planName}}</strong>."),
    variables: ["recipientName", "planName"]
  },
  {
    key: "subscription.canceled",
    name: "Abonnement annul\xE9",
    category: "subscription",
    subject: "Votre abonnement a \xE9t\xE9 annul\xE9",
    html: p(`Bonjour ${greet},`) + p("Votre abonnement est annul\xE9. Votre acc\xE8s reste actif jusqu'au {{accessUntil}}."),
  ...
  },
  {
    ...
    category: "subscription",
    subject: "Votre acc\xE8s est suspendu",
    html: p(`Bonjour ${greet},`) + p("L'acc\xE8s \xE0 votre espace est suspendu. R\xE9gularisez votre situation pour le r\xE9activer.") + btn("{{billingUrl}}", "R\xE9gulariser"),
  ...
  // ── Facturation / paiements ──────────────────────────────────────────────
  {
    ...
    html: p(`Bonjour ${greet},`) + p("Votre facture <strong>{{invoiceNumber}}</strong> d'un montant de {{amount}} est disponible. \xC9ch\xE9ance : {{dueDate}}.") + btn("{{invoiceUrl}}", "Voir la facture"),
    variables: ["recipientName", "invoiceNumber", "amount", "dueDate", "invoiceUrl"]
  },
  {
    key: "billing.payment_succeeded",
    name: "Paiement re\xE7u",
    category: "billing",
    subject: "Paiement confirm\xE9 \u2014 merci !",
    html: p(`Bonjour ${greet},`) + p("Nous avons bien re\xE7u votre paiement de {{amount}} pour la facture {{invoiceNumber}}. Merci !"),
    variables: ["recipientName", "amount", "invoiceNumber"]
  },
  {
    key: "billing.payment_failed",
    name: "\xC9chec de paiement",
    category: "billing",
    subject: "Votre paiement a \xE9chou\xE9",
    html: p(`Bonjour ${greet},`) + p("Le paiement de {{amount}} pour la facture {{invoiceNumber}} a \xE9chou\xE9. Merci de mettre \xE0 jour votre moyen de paiement.") + btn("{{billingUrl}}", "Mettre \xE0 jour"),
  ...
  },
  {
    ...
    html: p(`Bonjour ${greet},`) + p("La facture <strong>{{invoiceNumber}}</strong> ({{amount}}) reste impay\xE9e depuis le {{dueDate}}. Merci de proc\xE9der au r\xE8glement.") + btn("{{invoiceUrl}}", "R\xE9gler la facture"),
    variables: ["recipientName", "invoiceNumber", "amount", "dueDate", "invoiceUrl"]
  },
  {
    ...
    category: "billing",
    subject: "2e rappel : facture {{invoiceNumber}} impay\xE9e",
    html: p(`Bonjour ${greet},`) + p("Malgr\xE9 notre premier rappel, la facture <strong>{{invoiceNumber}}</strong> ({{amount}}) demeure impay\xE9e. Sans r\xE8glement sous {{graceDays}} jours, l'acc\xE8s pourra \xEAtre suspendu.") + btn("{{invoiceUrl}}", "R\xE9gler maintenant"),
  ...
  },
  {
    ...
    html: p(`Bonjour ${greet},`) + p("Dernier avis : la facture <strong>{{invoiceNumber}}</strong> ({{amount}}) est impay\xE9e. \xC0 d\xE9faut de r\xE8glement imm\xE9diat, l'acc\xE8s \xE0 votre espace sera suspendu.") + btn("{{invoiceUrl}}", "R\xE9gler imm\xE9diatement"),
  ...
  },
  {
    ...
    html: p(`Bonjour ${greet},`) + p("Un avoir <strong>{{creditNoteNumber}}</strong> de {{amount}} a \xE9t\xE9 \xE9mis sur votre compte.") + btn("{{invoiceUrl}}", "Voir l'avoir"),
    variables: ["recipientName", "creditNoteNumber", "amount", "invoiceUrl"]
  },
  // ── Support ──────────────────────────────────────────────────────────────
  {
    key: "support.ticket_received",
    name: "Demande de support re\xE7ue",
    category: "support",
    subject: "Nous avons bien re\xE7u votre demande #{{ticketRef}}",
    html: p(`Bonjour ${greet},`) + p("Votre demande <strong>#{{ticketRef}}</strong> a bien \xE9t\xE9 enregistr\xE9e. Un conseiller vous r\xE9pondra rapidement."),
    variables: ["recipientName", "ticketRef"]
  },
  {
    key: "support.agent_reply",
    name: "R\xE9ponse d'un conseiller",
    category: "support",
    subject: "R\xE9ponse \xE0 votre demande #{{ticketRef}}",
    html: p(`Bonjour ${greet},`) + p("Un conseiller a r\xE9pondu \xE0 votre demande <strong>#{{ticketRef}}</strong>.") + btn("{{conversationUrl}}", "Voir la r\xE9ponse"),
    variables: ["recipientName", "ticketRef", "conversationUrl"]
  },
  {
    key: "support.ticket_resolved",
    name: "Demande r\xE9solue",
    category: "support",
    subject: "Votre demande #{{ticketRef}} est r\xE9solue",
    html: p(`Bonjour ${greet},`) + p("Nous avons marqu\xE9 votre demande <strong>#{{ticketRef}}</strong> comme r\xE9solue. N'h\xE9sitez pas \xE0 nous r\xE9\xE9crire si besoin."),
    variables: ["recipientName", "ticketRef"]
  },
  // ── Système ──────────────────────────────────────────────────────────────
  {
    key: "system.quota_warning",
    name: "Quota bient\xF4t atteint",
    category: "system",
    subject: "Vous approchez de votre limite {{quotaName}}",
    html: p(`Bonjour ${greet},`) + p("Vous avez utilis\xE9 {{usagePercent}} de votre quota <strong>{{quotaName}}</strong>. Pensez \xE0 augmenter votre offre si n\xE9cessaire.") + btn("{{pricingUrl}}", "Voir les offres"),
  ...
  },
  {
    ...
    html: p(`Bonjour ${greet},`) + p("Votre quota <strong>{{quotaName}}</strong> est atteint. Certaines actions sont bloqu\xE9es jusqu'\xE0 augmentation de l'offre.") + btn("{{pricingUrl}}", "Augmenter mon offre"),
    variables: ["recipientName", "quotaName", "pricingUrl"]
  },
  {
    key: "system.generic_notification",
    name: "Notification g\xE9n\xE9rique",
    category: "system",
    subject: "{{subject}}",
  ...
  },
  {
    ...
    category: "marketing",
    subject: "Nouveaut\xE9 : {{featureName}}",
...
async function main() {
  ...
  try {
    ...
    console.log(`\u2705 seed-mail-templates : ${created} cr\xE9\xE9(s), ${skipped} d\xE9j\xE0 pr\xE9sent(s) sur ${DEFAULT_TEMPLATES.length}.`);
  } finally {
    await client.end();
  }
  process.exit(0);
...
main().catch((e) => {
  console.error("\u274C saas:seed-mail-templates :", e instanceof Error ? e.message : String(e));
  process.exit(1);
});
