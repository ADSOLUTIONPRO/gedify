...
// scripts/saas/check-settings.ts
function flag(name) {
...
async function main() {
  ...
  try {
    ...
    try {
      ...
      console.log("\u26A0\uFE0F  Table saas_settings absente \u2014 ex\xE9cutez `npm run db:push` (les valeurs par d\xE9faut s'appliquent).");
    }
    const sec = (k) => data?.[k] ?? {};
    ...
    console.log("R\xE9glages SaaS :");
    console.log(`   \u2022 Enregistr\xE9s en base : ${data ? "oui" : "non (d\xE9fauts)"}`);
    const pubSignup = features.publicSignup === true && signup.publicSignupEnabled === true;
    console.log(`   \u2022 Inscription publique : ${pubSignup ? "OUVERTE" : "ferm\xE9e"}${signup.inviteOnly ? " (sur invitation)" : ""}`);
    if (signup.publicSignupEnabled === true && features.publicSignup !== true) warns.push("Inscription publique activ\xE9e mais interrupteur global publicSignup OFF.");
    ...
    if (flag("EMAILS_ENABLED") && !(has("SMTP_HOST") && (has("SMTP_USER") || has("SMTP_USERNAME")) && (has("SMTP_PASSWORD") || has("SMTP_PASS")))) {
      warns.push("EMAILS_ENABLED=true mais configuration SMTP incompl\xE8te.");
    }
    if (!emails.supportEmail) warns.push("Email support non renseign\xE9.");
    if (!emails.billingEmail) warns.push("Email facturation non renseign\xE9.");
    ...
    console.log(`   \u2022 Chiffrement (KEK)   : ${has("ENCRYPTION_MASTER_KEY") ? "pr\xE9sente" : "absente"}`);
    const grace = Number(payment.graceDays ?? 0);
    console.log(`   \u2022 Politique paiement  : gr\xE2ce ${grace}j, relances ${payment.autoRemindersEnabled ? "auto" : "off"}`);
    if (grace <= 0) warns.push("D\xE9lai de gr\xE2ce paiement = 0.");
    console.log(`   \u2022 Support humain      : ${support.humanSupportEnabled ? "oui" : "non"}`);
    console.log(`   \u2022 Facturation         : pr\xE9fixes ${billing.invoicePrefix ?? "FAC"}/${billing.creditNotePrefix ?? "AVOIR"}, TVA ${billing.defaultVatRate ?? 20}%, ${billing.currency ?? "EUR"}`);
    if (!billing.invoicePrefix || !billing.creditNotePrefix) warns.push("Pr\xE9fixes de facturation manquants.");
    const off = Object.entries(features).filter(([, v]) => v === false).map(([k]) => k);
    if (off.length) console.log(`   \u2022 Fonctionnalit\xE9s coup\xE9es globalement : ${off.join(", ")}`);
    if (warns.length) {
      console.log("\nAvertissements :");
      for (const w of warns) console.log(`   \u26A0\uFE0F  ${w}`);
    } else console.log("\n\u2705 Aucune incoh\xE9rence d\xE9tect\xE9e.");
    console.log("\n\u2705 check-settings termin\xE9 (aucun secret affich\xE9).");
  } finally {
    await client.end();
  }
  process.exit(0);
...
main().catch((e) => {
  console.error("\u274C saas:check-settings :", e instanceof Error ? e.message : String(e));
  process.exit(1);
});
