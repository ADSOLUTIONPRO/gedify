...
var require_nodemailer = __commonJS({
  "node_modules/nodemailer/lib/nodemailer.js"(exports, module) {
    ...
    module.exports.createTransport = function(transporter, defaults2) {
      ...
      return new Mailer(transporter, options, defaults2);
    };
...
// scripts/saas/process-mail-queue.ts
...
var LIMIT = Number(process.env.MAIL_BATCH?.trim() || "50") || 50;
async function main() {
  if (!enabled()) {
    console.log("\u23ED\uFE0F  Mailing d\xE9sactiv\xE9 (EMAILS_ENABLED\u2260true) \u2014 rien \xE0 envoyer.");
    process.exit(0);
  }
  const cfg = smtpConfig();
  if (!cfg) {
    console.error("\u274C Configuration SMTP incompl\xE8te (SMTP_HOST/SMTP_USER/SMTP_PASSWORD/MAIL_FROM).");
    process.exit(1);
  }
  ...
  try {
    ...
    for (const row of rows) {
      ...
      try {
        const info = await transporter.sendMail({
          ...
          text: row.text_body ? String(row.text_body) : void 0
        });
    ...
    console.log(`\u2705 process-mail-queue : ${sent} envoy\xE9(s), ${failed} \xE9chec(s) sur ${rows.length} trait\xE9(s).`);
  } finally {
    await client.end();
  }
  process.exit(0);
...
main().catch((e) => {
  console.error("\u274C saas:process-mail-queue :", e instanceof Error ? e.message : String(e));
  process.exit(1);
});
