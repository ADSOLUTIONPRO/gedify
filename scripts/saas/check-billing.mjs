...
// scripts/saas/check-billing.ts
var REQUIRED = [
  ["company_name", "Soci\xE9t\xE9"],
...
async function main() {
  ...
  try {
    ...
    if (!profile) {
      console.log("   \u26A0\uFE0F  Aucun profil configur\xE9 (cr\xE9ez-le via /admin/saas/billing/profile).");
    } else {
      const missing = REQUIRED.filter(([k]) => !String(profile?.[k] ?? "").trim()).map(([, lbl]) => lbl);
      console.log(`   \u2022 Soci\xE9t\xE9           : ${profile.company_name ?? "\u2014"}`);
      console.log(`   \u2022 R\xE9gime TVA        : ${profile.vat_regime ?? "\u2014"}`);
      console.log(`   \u2022 Pr\xE9fixes          : ${profile.invoice_prefix ?? "FAC"} / ${profile.credit_note_prefix ?? "AVOIR"}`);
      console.log(`   \u2022 Prochains num\xE9ros : facture #${profile.next_invoice_number ?? 1} \xB7 avoir #${profile.next_credit_note_number ?? 1}`);
      console.log(missing.length ? `   \u26A0\uFE0F  Champs l\xE9gaux manquants : ${missing.join(", ")}` : "   \u2705 Champs l\xE9gaux essentiels pr\xE9sents.");
    }
    ...
    console.log(`   \u2022 \xC9mises             : ${issued}`);
    console.log(`   \u2022 Pay\xE9es             : ${paid}`);
    console.log(`   \u2022 Avoirs             : ${credits}`);
    console.log(`   \u2022 Lignes             : ${lines}`);
    if (orphanNum > 0) console.log(`   \u26A0\uFE0F  Factures \xE9mises SANS num\xE9ro : ${orphanNum}`);
...
main().catch((e) => {
  console.error("\u274C saas:check-billing :", e instanceof Error ? e.message : String(e));
  process.exit(1);
});
