...
var require_sasl = __commonJS({
  "node_modules/pg/lib/crypto/sasl.js"(exports, module) {
    ...
    function startSession(mechanisms, stream, scramMaxIterations = DEFAULT_MAX_SCRAM_ITERATIONS) {
      ...
      if (stream) candidates2.unshift("SCRAM-SHA-256-PLUS");
...
// scripts/saas/encryption-shared.ts
...
function legacyMedia(category) {
  return path.join(dataDir(), "media", category);
}
function candidates(category, filename) {
...
function resolveExisting(category, filename) {
...
function filesForDocument(id, storedFilename) {
  const out = [];
  if (storedFilename) {
...
async function loadDocuments(client) {
  ...
  return rows.map((r) => {
    ...
    return {
      ...
      storedFilename: typeof raw.storedFilename === "string" ? raw.storedFilename : null
    };
  });
}
async function loadTenantDeks(client, kek) {
  const map = /* @__PURE__ */ new Map();
  let rows = [];
  try {
    rows = (await client.query("SELECT tenant_id, wrapped_dek FROM tenant_encryption_keys")).rows;
  } catch {
    return map;
  }
  for (const r of rows) {
