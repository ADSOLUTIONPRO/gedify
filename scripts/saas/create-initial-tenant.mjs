...
// scripts/saas/create-initial-tenant.ts
var TENANT_ID = "azserver-staging";
var TENANT_NAME = "AzServer Staging";
var TENANT_SLUG = "azserver-staging";
var TENANT_PLAN = "internal";
var TENANT_STATUS = "active";
var DDL = `
...
async function main() {
  ...
  try {
    ...
    if (!adminRow) {
      console.error("\u274C Aucun utilisateur trouv\xE9 \u2014 cr\xE9ez d'abord un compte admin, puis relancez.");
      process.exit(1);
    }
    ...
    console.log("\u2705 saas:init-tenant termin\xE9 (idempotent)");
    console.log(`   \u2022 Tenant       : ${TENANT_ID} (${TENANT_NAME}) \u2014 ${tenantCreated ? "cr\xE9\xE9" : "d\xE9j\xE0 pr\xE9sent"}`);
    console.log(`   \u2022 Admin        : #${userId} ${adminRow.username ?? ""}${adminRow.email ? ` <${adminRow.email}>` : ""}`);
    console.log(`   \u2022 Membership   : ${membershipId} role=owner \u2014 ${membershipCreated ? "cr\xE9\xE9" : "d\xE9j\xE0 pr\xE9sent"}`);
    console.log(`   \u2022 TenantSettings: ${settingsCreated ? "cr\xE9\xE9es" : "d\xE9j\xE0 pr\xE9sentes"}`);
  } finally {
    await client.end();
  }
}
