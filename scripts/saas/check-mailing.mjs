...
// scripts/saas/check-mailing.ts
function has(k) {
  return Boolean(process.env[k]?.trim());
}
function flag(name) {
  const v = process.env[name]?.trim().toLowerCase();
  return v === "true" || v === "1" || v === "yes" || v === "on" ? "activ\xE9" : "d\xE9sactiv\xE9";
}
async function main() {
  console.log("Mailing :");
  console.log(`   \u2022 EMAILS_ENABLED : ${flag("EMAILS_ENABLED")}`);
  console.log(`   \u2022 SMTP_HOST      : ${has("SMTP_HOST") ? "pr\xE9sent" : "absent"}`);
  console.log(`   \u2022 SMTP_USER      : ${has("SMTP_USER") || has("SMTP_USERNAME") ? "pr\xE9sent" : "absent"}`);
  console.log(`   \u2022 SMTP_PASSWORD  : ${has("SMTP_PASSWORD") || has("SMTP_PASS") ? "pr\xE9sent (masqu\xE9)" : "absent"}`);
  console.log(`   \u2022 MAIL_FROM      : ${has("MAIL_FROM") || has("SMTP_FROM") ? "pr\xE9sent" : "absent"}`);
  console.log(`   \u2022 SMTP_PORT      : ${process.env.SMTP_PORT?.trim() || "(d\xE9faut 465)"}`);
  ...
  try {
    ...
    console.log("\nBase :");
    console.log(`   \u2022 Mod\xE8les (actifs)    : ${templates} (${enabledTpl})`);
    console.log(`   \u2022 File : en attente   : ${queued}`);
    console.log(`   \u2022 File : envoy\xE9s      : ${sent}`);
    console.log(`   \u2022 File : \xE9chou\xE9s      : ${failed}`);
    if (stuck > 0) console.log(`   \u26A0\uFE0F  Messages bloqu\xE9s en 'sending' (>15min) : ${stuck}`);
...
main().catch((e) => {
  console.error("\u274C saas:check-mailing :", e instanceof Error ? e.message : String(e));
  process.exit(1);
});
