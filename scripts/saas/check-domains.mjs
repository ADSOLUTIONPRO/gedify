...
async function main() {
  ...
  try {
    ...
    try {
      ...
      console.log("\u26A0\uFE0F  Table tenant_domains absente \u2014 ex\xE9cutez `npm run db:push`.");
      process.exit(0);
    }
    ...
    console.log(`   \u2022 \xC9chec              : ${by((r) => r.status === "failed")}`);
    console.log(`   \u2022 D\xE9sactiv\xE9s         : ${by((r) => r.status === "disabled")}`);
    console.log(`   \u2022 Non v\xE9rifi\xE9s       : ${by((r) => r.verification_status !== "verified")}`);
    ...
    const primaryByTenant = /* @__PURE__ */ new Map();
...
main().catch((e) => {
  console.error("\u274C saas:check-domains :", e instanceof Error ? e.message : String(e));
  process.exit(1);
});
