...
// scripts/saas/backfill-tenant-id.ts
var TENANT_ID = process.env.TENANT_ID?.trim() || "azserver-staging";
var TABLES = ["documents", "tags", "correspondents", "document_types", "folders"];
async function main() {
  ...
  try {
    ...
    for (const table of TABLES) {
      try {
        ...
        console.log(`   \u2022 ${table}: ${res.rowCount ?? 0} ligne(s) rattach\xE9e(s) \u2192 ${TENANT_ID}`);
      } catch (e) {
        console.warn(`   \u2022 ${table}: ignor\xE9e (${e instanceof Error ? e.message : String(e)})`);
      }
    }
    console.log(`\u2705 saas:backfill-tenant termin\xE9 (idempotent) \u2014 ${total} ligne(s) mises \xE0 jour au total.`);
  } finally {
    await client.end();
  }
}
