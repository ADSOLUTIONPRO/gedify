...
// scripts/saas/check-invitations.ts
async function main() {
  ...
  try {
    let total = -1;
    try {
      total = Number((await client.query("SELECT COUNT(*)::int n FROM tenant_invitations")).rows[0]?.n ?? 0);
    } catch {
      console.log("\u26A0\uFE0F  Table tenant_invitations absente \u2014 ex\xE9cutez `npm run db:push`.");
      process.exit(0);
    }
    const n = async (sql) => {
      try {
    ...
    console.log(`   \u2022 Accept\xE9es                   : ${await n("SELECT COUNT(*)::int n FROM tenant_invitations WHERE status='accepted'")}`);
    console.log(`   \u2022 Expir\xE9es                    : ${await n("SELECT COUNT(*)::int n FROM tenant_invitations WHERE status='expired'")}`);
    console.log(`   \u2022 Annul\xE9es                    : ${await n("SELECT COUNT(*)::int n FROM tenant_invitations WHERE status='canceled'")}`);
    const staleP = await n("SELECT COUNT(*)::int n FROM tenant_invitations WHERE status='pending' AND expires_at < now()");
    if (staleP > 0) console.log(`   \u26A0\uFE0F  Pending expir\xE9es (\xE0 traiter) : ${staleP}`);
    ...
    if (overSent > 0) console.log(`   \u26A0\uFE0F  Renvoy\xE9es >10 fois : ${overSent}`);
...
main().catch((e) => {
  console.error("\u274C saas:check-invitations :", e instanceof Error ? e.message : String(e));
  process.exit(1);
});
