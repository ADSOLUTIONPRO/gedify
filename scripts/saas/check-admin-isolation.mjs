...
// scripts/saas/check-admin-isolation.ts
async function main() {
  ...
  try {
    ...
    for (const table of ["documents", "tags", "correspondents", "document_types"]) {
      ...
      if (orphan > 0) {
        console.log(`   \u26A0\uFE0F  ${table} sans tenant_id : ${orphan} (lancez saas:attach-data)`);
        problems++;
      }
    }
    ...
    if (problems === 0) console.log("\n\u2705 Aucune fuite d'isolation d\xE9tect\xE9e.");
    else console.log(`
\u274C ${problems} probl\xE8me(s) d'isolation d\xE9tect\xE9(s).`);
...
main().catch((e) => {
  console.error("\u274C saas:check-admin-isolation :", e instanceof Error ? e.message : String(e));
  process.exit(1);
});
