...
// scripts/saas/check-security.ts
async function main() {
  ...
  try {
    ...
    let total = -1;
    try {
      total = Number((await client.query("SELECT COUNT(*)::int n FROM security_events")).rows[0]?.n ?? 0);
    } catch {
      console.log("\u26A0\uFE0F  Table security_events absente \u2014 ex\xE9cutez `npm run db:push`.");
      process.exit(0);
    }
    ...
    console.log("S\xE9curit\xE9 :");
    console.log(`   \u2022 \xC9v\xE9nements (total)         : ${total}`);
    console.log(`   \u2022 Critiques ouverts          : ${critOpen}`);
    console.log(`   \u2022 Avertissements ouverts     : ${warnOpen}`);
    console.log(`   \u2022 \xC9checs login 24h           : ${failed24}`);
    console.log(`   \u2022 Tentatives cross-tenant    : ${cross}`);
    console.log(`   \u2022 Acc\xE8s non autoris\xE9s (7j)   : ${unauth}`);
    ...
    try {
      ...
      if (rows.length) {
        console.log("\n   \u26A0\uFE0F  IP suspectes (\u22655 \xE9checs/h) :");
        for (const r of rows) console.log(`      - ${r.ip} : ${r.n} \xE9checs`);
      }
    } catch {
    }
    if (critOpen > 0) console.log(`
   \u2757 ${critOpen} alerte(s) critique(s) \xE0 traiter sur /admin/saas/security`);
    console.log("\n\u2705 check-security termin\xE9.");
  } finally {
    await client.end();
  }
  process.exit(0);
...
main().catch((e) => {
  console.error("\u274C saas:check-security :", e instanceof Error ? e.message : String(e));
  process.exit(1);
});
