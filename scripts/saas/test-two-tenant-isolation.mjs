...
// scripts/saas/test-two-tenant-isolation.ts
var A = "azserver-staging";
var B = "test-client";
var ID = {
  doc: { [A]: 990000101, [B]: 990000102 },
  tag: { [A]: 990000111, [B]: 990000112 },
  corr: { [A]: 990000121, [B]: 990000122 }
};
var ALL_DOC = Object.values(ID.doc);
var ALL_TAG = Object.values(ID.tag);
var ALL_CORR = Object.values(ID.corr);
async function main() {
  ...
  async function upsert(table, id, tenantId, raw) {
    await client.query(
  ...
  async function checkPair(table, ids, kind) {
    for (const [own, other] of [[A, B], [B, A]]) {
      ...
      if (visibleOwn !== 1) failures.push(`${kind}: t\xE9moin de ${own} non visible dans son propre tenant`);
      if (visibleCross !== 0) failures.push(`FUITE ${kind}: ${own} voit le t\xE9moin de ${other}`);
  ...
  try {
    ...
    console.log("V\xE9rifications d'isolation (own=1 attendu, cross=0 attendu) :");
    ...
    if (failures.length === 0) {
      console.log("\u2705 Isolation 2 tenants OK \u2014 aucune fuite d\xE9tect\xE9e.");
    } else {
