...
async function main() {
  ...
  console.log(`   \u2022 STRIPE_SECRET_KEY   : ${has("STRIPE_SECRET_KEY") ? "pr\xE9sente" : "absente"}`);
  console.log(`   \u2022 STRIPE_WEBHOOK_SECRET: ${has("STRIPE_WEBHOOK_SECRET") ? "pr\xE9sent" : "absent"}`);
  console.log(`   \u2022 SUCCESS/CANCEL URL  : ${has("STRIPE_SUCCESS_URL") ? "ok" : "d\xE9faut"}/${has("STRIPE_CANCEL_URL") ? "ok" : "d\xE9faut"}`);
  ...
  try {
    ...
    console.log(`   \u2022 \xC9v\xE9nements webhook trait\xE9s       : ${events}`);
    console.log("\n\u2705 check-stripe termin\xE9 (aucun appel Stripe effectu\xE9).");
  } finally {
    await client.end();
  }
  process.exit(0);
...
main().catch((e) => {
  console.error("\u274C saas:check-stripe :", e instanceof Error ? e.message : String(e));
  process.exit(1);
});
