...
// scripts/saas/create-manual-subscription.ts
import { randomUUID } from "node:crypto";
var STATUSES = ["trialing", "active", "past_due", "canceled", "unpaid", "paused", "incomplete"];
var DDL = `
...
async function main() {
  ...
  try {
    ...
    if (!t.rowCount) console.warn(`\u26A0\uFE0F  Tenant \xAB ${tenant} \xBB introuvable (l'abonnement sera quand m\xEAme cr\xE9\xE9).`);
    ...
    if (existing.rows[0]) {
      ...
      console.log(`\u2705 Abonnement mis \xE0 jour : ${tenant} \u2192 plan=${plan} status=${status}`);
    } else {
      await client.query(
        "INSERT INTO subscriptions(id, tenant_id, plan, status, provider) VALUES($1,$2,$3,$4,'manual')",
        [randomUUID(), tenant, plan, status]
      );
      console.log(`\u2705 Abonnement cr\xE9\xE9 : ${tenant} \u2192 plan=${plan} status=${status}`);
    }
  } finally {
    await client.end();
  }
...
main().catch((e) => {
  console.error("\u274C saas:create-manual-subscription :", e instanceof Error ? e.message : String(e));
  process.exit(1);
});
