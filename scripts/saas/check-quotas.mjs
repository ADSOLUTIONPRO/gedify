...
var PLAN_DEFAULTS = {
  ...
  pro: { maxUsers: 5, maxDocuments: 2e3, maxStorageMb: 5e3, ai: true, ocr: true, email: true, office: true },
  business: { maxUsers: 20, maxDocuments: 2e4, maxStorageMb: 5e4, ai: true, ocr: true, email: true, office: true },
...
async function main() {
  ...
  try {
    ...
    for (const t of tenants.rows) {
      ...
      console.log(`    features: ai=${ai} ocr=${ocr} email=${email} onlyoffice=${office}${over.length ? `   \u26A0\uFE0F D\xC9PASSEMENT: ${over.join(", ")}` : ""}`);
    }
    console.log("");
    if (overCount === 0) {
      console.log("\u2705 Aucun d\xE9passement de quota.");
      process.exit(0);
    } else {
      console.error(`\u274C ${overCount} tenant(s) en d\xE9passement de quota.`);
      process.exit(1);
    }
  } finally {
    await client.end();
