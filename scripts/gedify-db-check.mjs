...
async function main() {
  ...
  try {
    ...
    if (/role .* does not exist|password authentication failed/i.test(msg)) {
      console.error("   \u2192 utilisateur/mot de passe incorrects. Copie la cha\xEEne EXACTE depuis");
      console.error("     Coolify \u2192 service PostgreSQL \u2192 connexion interne.");
    } else if (/ENOTFOUND|getaddrinfo|ECONNREFUSED/i.test(msg)) {
      console.error("   \u2192 h\xF4te injoignable : Gedify et la base ne partagent pas le r\xE9seau Coolify.");
    } else if (/SSL|self-signed/i.test(msg)) {
      console.error("   \u2192 ajoute ?sslmode=require (ou ?sslmode=no-verify) \xE0 DATABASE_URL.");
    }
    process.exit(1);
  }
  try {
    const who = await client.query("select current_user as u, current_database() as d");
    console.log(`\u2705 Connect\xE9 \u2014 utilisateur="${who.rows[0].u}", base="${who.rows[0].d}"`);
    try {
      ...
      console.error(`\u26A0\uFE0F  Droits insuffisants pour cr\xE9er des tables : ${e instanceof Error ? e.message : String(e)}`);
      console.error("   \u2192 utilise la base dont cet utilisateur est PROPRI\xC9TAIRE, ou accorde les droits sur le sch\xE9ma public.");
      process.exitCode = 1;
    }
  } finally {
    await client.end();
